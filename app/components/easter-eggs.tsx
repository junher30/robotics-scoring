...
// Para agregar otro, copia un bloque: words (sin tildes, en minúscula), pieces (emojis que caen o suben), title y text.
...
export default function EasterEgg({query}:{query:string}){
 ...
 if(!egg)return null;
 return <>
  <div key={egg.id} className={`${s.rain} ${egg.motion==='rise'?s.rise:s.fall}`} aria-hidden="true">{pieces.map((p,i)=><span key={i} style={{left:`${p.left}%`,fontSize:p.size,animationDelay:`${p.delay}s`,animationDuration:`${p.duration}s`,'--sway':`${p.sway}px`} as CSSProperties}>{p.char}</span>)}</div>
  <aside className={s.card} role="status"><span className={s.icon} aria-hidden="true">{egg.pieces[0]}</span><div><small>🥚 Easter egg encontrado</small><strong>{egg.title}</strong><p>{egg.text}</p></div></aside>
 </>;
}
