...
export async function deleteJudge(id:string,version:string,_previous:DeleteJudgeState,form:FormData):Promise<DeleteJudgeState>{
 ...
 try{
  ...
  // Libera el correo original para que pueda volver a invitarse; el perfil sigue conservando el historial de puntuaciones.
  try{const admin=createAdminClient();await admin.auth.admin.updateUserById(id,{email:`deleted.${id}@deleted.invalid`,email_confirm:true});}catch{}
