...
export default async function Page({searchParams}:{searchParams:Promise<{estado?:string;pagina?:string}>}){
 ...
 return <EventShell><header className={s.titleRow}><div className={s.title}><p>TUS COMPETENCIAS</p><h1>Eventos.</h1><span>{profile.role==='SUPER_ADMIN'?'Gestiona todos los eventos de RoboScore.':'Gestiona los eventos que has creado o que otros administradores compartieron contigo.'}</span></div><Link href="/admin/eventos/nuevo" className={s.primary}>+ Nuevo evento</Link></header>
