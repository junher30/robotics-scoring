...
export type JudgeCategory=Omit<Assignment,'teams'|'id'>&{teams:{id:string;name:string;institution:string;robot_name:string|null;scored_count:number;total:number|null}[]};
export type JudgeTeam=Omit<Assignment,'teams'|'id'>&{team:{id:string;name:string;institution:string;status:string};challenges:Challenge[];scores:Score[]};
