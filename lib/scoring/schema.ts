...
export const teamSchema=z.object({category_id:z.string().uuid('Selecciona una categoría.'),name:z.string().trim().min(1,'Escribe el nombre.').max(150),institution:z.string().trim().min(1,'Escribe la institución.').max(200),robot_name:z.string().trim().max(150),team_number:z.string().trim().regex(/^\d*$/,'Escribe un número entero.').transform(v=>v===''?null:Number(v)).refine(v=>v===null||(v>0&&v<=100000),'El número debe ser mayor que cero.'),participants:z.string().max(2000),active:z.enum(['true','false'])});
...
export type Team={id:string;event_id:string;category_id:string;name:string;institution:string;robot_name:string|null;team_number:number|null;participant_names:string[];status:'PENDING'|'APPROVED'|'ACTIVE'|'REJECTED';updated_at:string};
...
export type Board={has_scores:boolean;config:{rule:ScoringRule;challenge_count:number}|null;challenges:Challenge[];teams:{id:string;name:string;institution:string;team_number:number|null;scored_count:number;total:number|null;position:number|null;scores:Score[]}[]};
